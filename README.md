...
> crates.io：**0.1.2 已上架** — [`plugctx`](https://crates.io/crates/plugctx) / [`plugctx-derive`](https://crates.io/crates/plugctx-derive)。曾用名 `pluggable` 已被无关方占用，勿与 crates.io 上其他 `pluggable` 混淆。已发布版本的 README 绑在该版本 `.crate` 上，改文案须 bump（例如 0.1.1 → 0.1.2）再 publish；详见 [`docs/publishing.md`](docs/publishing.md)。
...
- 已上架：`plugctx`、`plugctx-derive` **0.1.2**（0.1.0 / 0.1.1 仍保留；license / description / documentation；repository 见 [`docs/publishing.md`](docs/publishing.md)）
