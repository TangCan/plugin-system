...
**原生「卸载」= 先逻辑卸载（撤销 Context 注册与 Effect），再物理卸载（Drop `libloading::Library` → `dlclose` / `FreeLibrary`）**。热插拔 = load → use → dispose → load，不提供 `reload()`。Windows 映射期间可能锁文件，须先 dispose 或换路径。  
