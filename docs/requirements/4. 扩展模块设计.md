...
**原生「卸载」= 逻辑卸载**（撤销 Context 注册与 Effect），**默认不以 `dlclose` 为正确性前提**（FR25）。  
**WASM「卸载」= 实例级显式 close/free**，释放扩展侧运行时资源（FR26）。
...
*   native：动态库映射使用 `ManuallyDrop<Library>` **保留在进程内**。
...
sequenceDiagram
    ...
    alt ABI 不匹配
        ...
        User->>Context: start / dispose（逻辑卸载）
    end
```
...
*   第三方不可信或需真销毁映射时，优先 WASM 实例关闭，而非 native `dlclose`。
*   **逻辑卸载 ≠ `dlclose`**：native 正确性建立在撤销 Context 注册与 Effect 上。
