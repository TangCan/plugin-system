...
| 原生动态加载 | `dynamic-native` | `libloading`、`plugin-api` | C ABI 加载；dispose 后物理卸载（dlclose） |
...
| `dynamic-native` | 关闭 | `libloading`, `plugin-api` | 原生 C ABI 动态库；dispose 后 dlclose；`PLUGIN_ABI_VERSION` |
