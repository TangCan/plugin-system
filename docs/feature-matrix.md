...
5. **版本号（FR54）**：工作区 `version = "0.1.0"` 可同时承载 0.2.0 **能力清单**（见 CHANGELOG）；文档切片 ≠ 强制 bump 到 `0.2.0` 字符串。加性 feature 留同 `0.y`；breaking 才 bump。详见 [`publishing.md`](publishing.md)「0.y 版本策略」。
